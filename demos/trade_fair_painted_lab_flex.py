...
Test-instrumentation note:
  ...
  height after the larger main draw. On Flex 1k (min 5 uL) every
  workflow volume in this protocol (smallest is 20 uL) splits
  cleanly; no compat warnings expected. On the OT-2 port the
  pipette minimum is 20 uL and you may want to flip the RTP OFF
  for the run.
...
def run(protocol: protocol_api.ProtocolContext):
        ...
        for tip_label, lane, cols, overlay in blocks:
            ...
            if overlay == "blue_overlay":
                ...
                for col in cols:
                    ...
                    dispense_and_lift(flex_m, block_ul, plate.columns()[col][0])
                    flex_m.mix(2, MIX_UL, plate.columns()[col][0].bottom(2))
                finish_multi()
    ...
    if _trailing_enabled:
        ...
        _trailed_extra_min = _enabled_calls * TRAILING_DELAY_S / 60.0
        protocol.comment(
            f"Estimated run time: ~{_est_total_min + _trailed_extra_min:.0f} min "
            f"(workflow ~{_enabled_plate_minutes} min + delays "
            f"~{_delay_minutes:.0f} min + trailed-wrapper latency "
            f"~{_trailed_extra_min:.0f} min from {_enabled_calls} aspirate/dispense pairs)."
        )
    else:
        protocol.comment(
    ...
    protocol.comment(
        ...
        f"dispense={_trailed_counts['disp']} (each step adds "
        f"~{TRAILING_DELAY_S}s of pause when TRAILING_TEST_ENABLED=True)"
    )
