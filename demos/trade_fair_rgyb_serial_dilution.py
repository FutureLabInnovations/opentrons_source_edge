"""
OT-2 Trade Fair Demo  -  RGYB Serial Dilution (12-channel reservoir port)
=========================================================================

Direct port of the lab's NCBL v0.973 RGYB serial-dilution protocol,
restructured to use a NEST 12-channel reservoir instead of the
original Falcon-tube racks. The proven 150 / 75 / 75 / mix(4, 75)
serial-dilution math, the per-source liquid-height tracking, the
post-dispense lift-off "touch-off without touch_tip" pattern, and the
wash-and-return tip discipline are all preserved.

Workflow (3 plates, 3 colours):
  * Plate 1 (slot 1): Red   stock, 1:2 serial dilution cols 1->12
  * Plate 2 (slot 2): Yellow stock, 1:2 serial dilution cols 1->12
  * Plate 3 (slot 3): Blue  stock, 1:2 serial dilution cols 1->12

Per plate the choreography is:
  1. Single-channel: load 75 uL diluent into every well of cols 2-12.
  2. Single-channel: load 150 uL stock into every well of col 1.
  3. Wash that tip in the wash lane, then RETURN it (never drop).
  4. Multi-channel: aspirate 75 uL from col 1, dispense + mix into
     col 2; repeat across cols 1 -> 12. That's the 1:2 dilution.
  5. Wash the multi-channel tip column in the wash lane, RETURN them.

Tip discipline (per the booth-wide rule):
  ...
  * The single-channel tip used for diluent + stock is contaminated
    with stock dye by the end of step 2; the wash before return
    rinses it back to a near-clean state.
  * Each colour gets its own single-channel tip and its own multi-
    channel tip column - no cross-colour reuse.
...
Labware:
  * 3 x thermofisher_96_wellplate_250ul   (custom labware, slots 1, 2, 3)
  * 1 x nest_12_reservoir_15ml            (slot 4 - reservoir below)
  * 1 x opentrons_96_tiprack_300ul        (slot 6)

Reservoir layout (slot 4, NEST 12-channel):
...
==============================================================================
  ...
  3. Plate 1 (red)   -> diluent fill -> stock fill -> wash + return tip
                     -> multi-channel serial dilute -> wash + return tips.
  4. Plate 2 (yellow) - same choreography.
  5. Plate 3 (blue)   - same choreography.
...
metadata = {
    "protocolName": "Trade Fair - RGYB Serial Dilution (12-well reservoir port)",
    "author": "FLi / 12-channel adaptation",
    "description": (
        "OT-2 port of the NCBL v0.973 RGYB serial-dilution protocol. "
        "12-channel NEST reservoir instead of Falcon tubes; LaneTracker + "
        "WellTracker for liquid-height tracking; wash-and-return tip "
        "discipline (no tips dropped to trash)."
    ),
    "source": "NCBL v0.973 (FLi)",
}

# define_liquid() and load_liquid() need apiLevel >= 2.14; 2.16 matches the
# proven internal protocol's apiLevel exactly.
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    # Labware
    # -----------------------------------------------------------------------
    tips_p300 = protocol.load_labware("opentrons_96_tiprack_300ul", 6)
    reservoir = protocol.load_labware("nest_12_reservoir_15ml",     4)

    target_plate_1 = protocol.load_labware(PLATE_LOAD_NAME, 1, "plate-1 red")
    target_plate_2 = protocol.load_labware(PLATE_LOAD_NAME, 2, "plate-2 yellow")
    target_plate_3 = protocol.load_labware(PLATE_LOAD_NAME, 3, "plate-3 blue")
    ...
    p300s = protocol.load_instrument(
        "p300_single_gen2", "left",  tip_racks=[tips_p300]
    )
    p300m = protocol.load_instrument(
        "p300_multi_gen2",  "right", tip_racks=[tips_p300]
    )
    ...
    def run_one_plate(plate, colour_name, single_tip_index, multi_tip_col_index):
        ...
        single_tip_well = tips_p300.wells()[single_tip_index]
        multi_tip_top   = tips_p300.wells()[multi_tip_col_index * 8]
        # (multi-channel pipette picks a whole column starting at the top
        ...
        # ----- 1. Single-channel: diluent into every well of cols 2..12 -----
        p300s.pick_up_tip(single_tip_well)
        ...
        for i in range(8, 96):  # wells 8..95 in column-major order = cols 2..12
            ...
            trailed_aspirate(p300s, DILUENT_UL, src_well.bottom(src_z))
            trailed_dispense(p300s, DILUENT_UL, dst_well.top(z=-5))
            lift_z = well_trackers[dst_well].add(DILUENT_UL)
            p300s.move_to(dst_well.bottom(z=lift_z))

        # ----- 2. Single-channel: stock into every well of col 1 -----
        protocol.comment(f"  Loading {STOCK_UL} uL {colour_name} stock into col 1 (8 wells)")
        for i in range(8):
            dst_well = plate.wells()[i]
            src_z    = colour_tracker.aspirate_height(STOCK_UL)
            trailed_aspirate(p300s, STOCK_UL, colour_tracker.well.bottom(src_z))
            trailed_dispense(p300s, STOCK_UL, dst_well.top(z=-5))
            lift_z   = well_trackers[dst_well].add(STOCK_UL)
            p300s.move_to(dst_well.bottom(z=lift_z))

        # ----- 3. Wash the single-channel tip, RETURN it (never drop) -----
        protocol.comment("  Washing single-channel tip + returning to rack")
        wash_tip(p300s)
        p300s.return_tip()

        # ----- 4. Multi-channel: 1:2 serial dilution cols 1 -> 12 -----
        protocol.comment(
            f"  Multi-channel serial dilution: {XFER_UL} uL transfer "
            f"+ mix({MIX_REPS}, {MIX_UL}) across 11 column transfers"
        )
        p300m.pick_up_tip(multi_tip_top)
        temp_well = 0   # index of column-1 top (A1)
        for _ in range(11):
            src_well = plate.wells()[temp_well]
            temp_well += 8
            dst_well = plate.wells()[temp_well]
            trailed_aspirate(p300m, XFER_UL, src_well.bottom(z=2))
            trailed_dispense(p300m, XFER_UL, dst_well.top(z=-5))
            ...
            p300m.move_to(dst_well.bottom(z=lift_z))

        # ----- 5. Wash the multi-channel tip column, RETURN it -----
        protocol.comment("  Washing multi-channel tips + returning to rack")
        wash_tip(p300m)
        p300m.return_tip()
