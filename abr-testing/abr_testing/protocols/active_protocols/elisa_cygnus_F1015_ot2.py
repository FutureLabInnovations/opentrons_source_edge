...
def run(protocol: protocol_api.ProtocolContext) -> None:
    ...
    # -------------------------------------------------------------------------
    # Standards in A1..A6, blank in A7, samples in B1..B4 of the tube rack.
    blank_tube = tube_rack.wells_by_name()["A7"] if INCLUDE_BLANK else None
    standard_tubes = [
        tube_rack.wells_by_name()[f"A{i + 1}"] for i in range(NUM_STANDARDS)
    ]
    sample_tubes = [
        tube_rack.wells_by_name()[f"B{i + 1}"] for i in range(num_samples)
    ]
